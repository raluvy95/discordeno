...
export function enableResharderPlugin<G extends GatewayManager>(
  ...
  options: ResharderPluginOptions,
): GatewayManagerWithResharding<G> {
  const gateway = manager as GatewayManagerWithResharding;

  gateway.resharder = {
    // Properties
    enabled: options.enabled ?? true,
    useOptimalLargeBotSharding: options.useOptimalLargeBotSharding ?? true,
    checkIntervalMilliseconds: options.checkIntervalMilliseconds ?? 28800000,
    ...
    start: async () => {
      if (gateway.resharder.intervalId !== undefined) {
        throw new Error("[RESHARDER] Cannot activate the resharder more than one time.");
      }

      gateway.resharder.intervalId = setInterval(async () => {
        gateway.debug("GW DEBUG", "[RESHARDER] Checking if resharding is needed.");
        if (!gateway.resharder.enabled) {
          return gateway.debug("GW DEBUG", "[RESHARDER] Resharding is disabled.");
        }

        const result = await gateway.resharder.getGatewayBot();

        const percentage = ((result.shards - gateway.manager.totalShards) / gateway.manager.totalShards) * 100;
        // Less than necessary% being used so do nothing
        if (percentage < gateway.resharder.percentage) return;
        ...
        return gateway.resharder.reshard(result);
      }, gateway.resharder.checkIntervalMilliseconds);
    },

    getGatewayBot: options.getGatewayBot,

    reshard: async (payload: GatewayBot) => {
      gateway.debug("GW DEBUG", "[Resharding] Starting the reshard process.");
      ...
      if (gateway.resharder.useOptimalLargeBotSharding) {
        // gateway.debug("GW DEBUG", "[Resharding] Using optimal large bot sharding solution.");
        tmpManager.manager.totalShards = tmpManager.calculateTotalShards();
      }

      tmpManager.spawnShards();
  ...
  // If it is enabled, we start in the next interval
  if (gateway.resharder.enabled) {
    gateway.debug("GW DEBUG", `[RESHARDER] Starting in ${gateway.resharder.checkIntervalMilliseconds}ms.`);

    setTimeout(() => {
      gateway.resharder.start();
    }, gateway.resharder.checkIntervalMilliseconds);
  }
...
export interface ResharderPluginOptions {
  /** Whether or not the resharder is enabled. */
  enabled: boolean;
  /**
  ...
  /** Handler that by default will check to see if resharding should occur. Can be overridden if you have multiple servers and you want to communicate through redis pubsub or whatever you prefer. */
  start: () => Promise<void>;
  /** Handler which is used to fetch the current gateway information of the bot. Highly recommended to use this to communicate to your rest proxy. */
  getGatewayBot: () => Promise<GatewayBot>;
  /** Handler that will begin resharding. */
  reshard: (gatewayBot: GatewayBot) => Promise<GatewayManager>;
}

export type GatewayManagerWithResharding<G extends GatewayManager = GatewayManager> =
  & G
  & {
    resharder: ResharderPluginOptions & {
